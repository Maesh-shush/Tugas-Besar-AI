...
def _build_alasan(
    ...
    if not pred_anomali:
        return (
            f"Transaksi ditandai sebagai anomali karena markup harga (+{selisih:.1f}%) melebihi ambang batas ({threshold:.0f}%), "
            f"meskipun model ML menilai valid (probabilitas: {prob_anomali:.0%}).{pola_pj}"
        )
...
def _classify_item(item: dict, model_bundle: Optional[dict], default_threshold: float) -> dict:
    ...
    if ml_ok:
        try:
            ...
            is_over_threshold = selisih > threshold

            if pred_anomali or is_over_threshold:
                status = "anomali"
...
def analyze_with_ml(items: list, model_bundle: dict = None, threshold: float = 20) -> list:
    ...
    for result in results:
        if result.get("used_ml"):
            ...
            pred_ml = result.pop("pred_anomali")
            th = result.pop("threshold")
            pj = result.pop("pj")
            result.pop("used_ml")
            if result["status"] == "valid":
                result["alasan"] = _build_alasan("valid", result["selisih_persen"], prob, th, pj, 0, pred_ml)
            else:
