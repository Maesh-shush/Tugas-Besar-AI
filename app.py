...
if file_pengadaan and file_referensi:
    ...
    if "analysis_results" in st.session_state:
                ...
                for idx, p in enumerate(flagged_pjs):
                    with cols_pj[idx % min(len(flagged_pjs), 4)]:
                        st.markdown(
                            ...
                            f'<div style="font-size:1rem; font-weight:700; color:#721c24;">🚩 {p}</div>'
        ...
        for i, (m, r) in enumerate(zip(items_to_analyze, results)):
            ...
            if m["pj"] != "-":
                ...
                if pj_score >= threshold_pj:
                    pj_html = f'&nbsp;|&nbsp; 👤 PJ: <span style="color:#dc3545;font-weight:bold;">🚩 {m["pj"]} (Skor: {pj_score})</span>'
                else:
                    pj_html = f'&nbsp;|&nbsp; 👤 PJ: <b>{m["pj"]}</b>'
            
            tgl_html = ('&nbsp;|&nbsp; 📅 ' + m["tgl"])                 if m["tgl"] != "-" else ""

            st.markdown(
                '<div class="result-box ' + status + '">'
                '<div class="item-name">' + str(i+1) + '. ' + m["nama_beli"] + ' &nbsp;' + badge + risiko_html + '</div>'
                '<div class="item-meta">'
                ...
                + pj_html + tgl_html +
                '</div>'
                '<div class="match-info">🔗 Cocok dengan: <i>"' + str(m["nama_match"]) + '"</i> (' + str(m["score"]) + '%) — Sumber: ' + str(m["sumber"]) + '</div>'
                '<div class="reason-text">💬 <i>' + alasan + '</i></div>'
                '</div>',
                unsafe_allow_html=True
            )
        ...
        if filter_opt in ["Semua", "Tidak Ditemukan"] and not_found:
            st.markdown("---")
            for m in not_found:
                pj_html = ('&nbsp;|&nbsp; 👤 PJ: <b>' + m["pj"] + '</b>') if m["pj"] != "-" else ""
                st.markdown(
                    '<div class="result-box notfound">'
                    '<div class="item-name">⚠️ ' + m["nama_beli"] + ' &nbsp;<span class="badge-notfound">TIDAK DITEMUKAN</span></div>'
